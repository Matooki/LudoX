...
public class RedPieceScript : MonoBehaviour
{
    public Rigidbody rb;
    ...
    void Update()
    {
        ...
        redWin = GameManager.redWin;
    } 
    void OnMouseOver(){
        if(Input.GetMouseButtonDown(0) && inRed && diceValue == 6 && isTurn && !canRoll){
            transform.position  = gb.GetComponent<Rigidbody>().position;
            piecePos = 0;
            GameManager.turnCount = 1;
            DiceScript.canRoll = true;
        }
        else if(inRed && diceValue != 6 && isTurn && !canRoll)
        {
            GameManager.turnCount = 1;
            DiceScript.canRoll = true;
        }
        
        if(Input.GetMouseButtonDown(0) && !inRed && isTurn && !canRoll){
    ...
    {
        if (other.tag == "RedFinish")
        {
            GameManager.redWin -= 1;
        }
